...
class FeatureLoss(nn.Module):
    def __init__(self, input_size, size_average=True, n_not_matching=5, min_distance_for_missmatches=200,
                 non_match_weight=None, loss_type='L1', feat_dist_threshold_nomatch=0.5, feat_dist_threshold_match=0.0):
        ...
        # self.class_inds_to_remove = class_inds_to_remove #This feature have
        # been moved to an external function
    ...
    def forward(self, inputs_ref, inputs_other, inds_ref, inds_other, weights):
        max_distance = math.sqrt(
            (inputs_ref.size(2)**2 + inputs_ref.size(3)**2))
        ...
        for b in range(inputs_ref.size(0)):
            ...
            valid_inds = (b_inds_ref[0,
                                     :] < inputs_ref.size(3)) & (b_inds_ref[0,
                                                                            :] >= 0) & (b_inds_ref[1,
                                                                                                   :] < inputs_ref.size(2)) & (b_inds_ref[1,
                                                                                                                                          :] >= 0) & (b_inds_other[0,
                                                                                                                                                                   :] < inputs_ref.size(3)) & (b_inds_other[0,
                                                                                                                                                                                                            :] >= 0) & (b_inds_other[1,
                                                                                                                                                                                                                                     :] < inputs_ref.size(2)) & (b_inds_other[1,
                                                                                                                                                                                                                                                                              :] >= 0)
            ...
            lin_inds_ref_missmatch = torch.zeros(
                self.n_not_matching * n_matches,
                dtype=lin_inds_ref.dtype).to(
                lin_inds_ref.device)
            lin_inds_other_missmatch = torch.zeros(
                self.n_not_matching * n_matches,
                dtype=lin_inds_other.dtype).to(
                lin_inds_other.device)
            weights_missmatch = torch.zeros(
                self.n_not_matching * n_matches,
                dtype=weights_this.dtype).to(
                weights_this.device)
            # loop through all matches, for each match add a randomized
            # missmatch for which we train the features to be different
            current_ind = 0
            for match_ind in range(n_matches):
                ...
                while n_added < self.n_not_matching:
                    ...
                    if (distance_between_matches >
                            min_distance_for_missmatches_scaled):
                        # add missmatch
            ...
            if self.loss_type == 'L2':
                loss_match = torch.max(torch.sum((inp_ref_flat[b,
                                                               :,
                                                               lin_inds_ref] - inp_other_flat[b,
                                                                                              :,
                                                                                              lin_inds_other])**2,
                                                 0),
                                       torch.tensor(feat_dist_thresh_match,
                                                    dtype=torch.float32,
                                                    device=weights_this.device))
                loss_missmatch = torch.min(torch.sum((inp_ref_flat[b,
                                                                   :,
                                                                   lin_inds_ref_missmatch] - inp_other_flat[b,
                                                                                                            :,
                                                                                                            lin_inds_other_missmatch])**2,
                                                     0),
                                           torch.tensor(feat_dist_thresh_nomatch,
                                                        dtype=torch.float32,
                                                        device=weights_this.device))
            elif self.loss_type == 'L1':
                loss_match = torch.max(torch.sum(torch.abs(inp_ref_flat[b,
                                                                        :,
                                                                        lin_inds_ref] - inp_other_flat[b,
                                                                                                       :,
                                                                                                       lin_inds_other]),
                                       ...
                                       torch.tensor(feat_dist_thresh_match,
                                                    dtype=torch.float32,
                                                    device=weights_this.device))
                loss_missmatch = torch.min(torch.sum(torch.abs(inp_ref_flat[b,
                                                                            :,
                                                                            lin_inds_ref_missmatch] - inp_other_flat[b,
                                                                                                                     :,
                                                                                                                     lin_inds_other_missmatch]),
                                                     0),
                                           torch.tensor(feat_dist_thresh_nomatch,
                                                        dtype=torch.float32,
                                                        device=weights_this.device))
                ...
                # max(m_p - dt'ds,0)
                loss_match = torch.max(feat_dist_thresh_match -
                                       torch.sum(inp_ref_flat[b, :, lin_inds_ref] *
                                                 inp_other_flat[b, :, lin_inds_other], 0), torch.tensor(0., dtype=torch.float32, device=weights_this.device))
                # max(dt'ds - m_n,0)
                loss_missmatch = torch.max(torch.sum(inp_ref_flat[b,
                                                                  :,
                                                                  lin_inds_ref_missmatch] * inp_other_flat[b,
                                                                                                           :,
                                                                                                           lin_inds_other_missmatch],
                                                     0) - feat_dist_thresh_nomatch,
                                           torch.tensor(0.,
                                                        dtype=torch.float32,
                                                        device=weights_this.device))
