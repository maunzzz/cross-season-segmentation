...
class CorrRandomCrop(object):
    ...
    def __call__(self, img1, img2, pts1, pts2, weights):
        ...
        if img1.size[0] < self.size[0] or img1.size[1] < self.size[1]:  # needs padding
            ...
            padding = (delta_x // 2, delta_y // 2, delta_x -
                       (delta_x // 2), delta_y - (delta_y // 2))
        ...
        in_im = (pts1[0,
                      :] >= 0) & (pts1[0,
                                       :] + .5 < self.size[1]) & (pts2[0,
                                                                       :] >= 0) & (pts2[0,
                                                                                        :] + .5 < self.size[1]) & (pts1[1,
                                                                                                                        :] >= 0) & (pts1[1,
                                                                                                                                         :] + .5 < self.size[0]) & (pts2[1,
                                                                                                                                                                         :] >= 0) & (pts2[1,
                                                                                                                                                                                          :] + .5 < self.size[0])
        pts1 = pts1[:, in_im]
        pts2 = pts2[:, in_im]
